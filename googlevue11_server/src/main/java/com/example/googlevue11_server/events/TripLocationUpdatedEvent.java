...
@ServerEndpoint("/broadcast/trip-location-updated")
public class TripLocationUpdatedEvent {
    ...
    public void broadcastTripLocationUpdated(Trip trip, User user) {
        JsonObject data = createBroadcastData(trip, user);
        broadcastToPassengerChannel(data, Math.toIntExact(user.getId()));
    }
}
